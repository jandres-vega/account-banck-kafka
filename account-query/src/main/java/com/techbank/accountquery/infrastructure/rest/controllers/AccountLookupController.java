package com.techbank.accountquery.infrastructure.rest.controllers;


import com.techbank.accountquery.application.dto.AccountLookupResponse;
import com.techbank.accountquery.application.dto.EquityType;
import com.techbank.accountquery.application.queries.FindAccountBalanceQuery;
import com.techbank.accountquery.application.queries.FindAccountByHolderQuery;
import com.techbank.accountquery.application.queries.FindAccountByIdQuery;
import com.techbank.accountquery.application.queries.FindAllAccountQuery;
...
public class AccountLookupController {
    ...
    public ResponseEntity<AccountLookupResponse> getAccountById(@PathVariable(value = "id") String id) {
        try {
            ...
            var response  = AccountLookupResponse.builder()
                    .accounts(accounts)
                    .message(MessageFormat.format("Failed to complete get account by id request for id {0}", id))
                    .build();
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception e) {
    ...
    public ResponseEntity<AccountLookupResponse> getAccountWithBalance(
            @PathVariable(value = "equalityType") EquityType equalityType,
            @PathVariable(value = "balance") double balance) {
        try {
            List<BankAccount> accounts = queryDispatcher.send(new FindAccountBalanceQuery(equalityType, balance));
            ...
            var safeErrorMessage = "Failed to complete get account withs balance request ";
            logger.log(Level.SEVERE, safeErrorMessage, e);
            return new ResponseEntity<>(new AccountLookupResponse(safeErrorMessage), HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
